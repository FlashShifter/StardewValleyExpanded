...
namespace StardewValleyExpanded
{
    ...
    public static class CustomCauldronEffects
    {
        ...
        private static Dictionary<string, List<Vector2>> WaterfallTiles { get; } = new Dictionary<string, List<Vector2>>()
        {
            ...
            {
                "Custom_FerngillRepublicFrontier", new List<Vector2>()
                {
                    new Vector2(148, 27),
                    new Vector2(148, 27),
                    new Vector2(149, 27),
        ...
        private static List<Vector2> GetConditionalWaterfallTiles(string locationName)
        {
            ...
            {
                if (Game1.whichFarm == Farm.default_layout && Helper.ModRegistry.IsLoaded("flashshifter.GrandpasFarm")) //if the player is using Grandpa's Farm
                {
            ...
            if (locationName == "Farm" && Game1.GetFarmTypeID() == "FrontierFarm")
                {
                    return new List<Vector2>()
                    {
                        //waterfall 1
            ...
            {
                if (Helper.ModRegistry.IsLoaded("Entoarox.ExtendedMinecart") == false) //if the player is NOT using Extended Minecart
                {
                    return new List<Vector2>()
                    {
                        new Vector2(29, 5),
